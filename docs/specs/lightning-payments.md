...
## Payment/evidence correlation and local plugin events

Payment recovery IDs remain private bearer capabilities. `RequestTerms.exchange_id`
is a separate optional public correlation ID, minted by the host's existing
OpenAI exchange scope. It matches that scope's `openai.exchange.v1` events; it is
not reconstructed from invoice contents or supplied by the HTTP caller. The
provider treats this correlation as a caller assertion, not authenticated proof
of an exchange. It does not equate the provider's independently minted inference
exchange with the payer's exchange. Missing correlation on older records remains
absent. Standalone wallet sends do not invent an inference exchange.

Plugins declaring `payment.lifecycle.v1` receive host-local JSON channel messages
with `correlation_id = exchange_id`. These are observations, not signed receipts:

- `version: 1`, `event_id`, `exchange_id`, `role` (`payer` or `provider`),
  `phase`, `evidence`, `terms_digest`, nullable `segment`, `invoice_ref`, and
  `amount_msat`.
- `terms_accepted`: payer authorization, labelled `local_authorization`.
- `invoice_issued`: persisted provider invoice, or a validated invoice prepared
  for payment on the payer. Labelled `provider_claim`; segment 0 is input, 1 output.
- `invoice_settled`: durable terminal success, labelled `wallet_reported`.
  HTLC `claiming`, pending, failed, and unknown outcomes never imply settlement.
- `final_amount`: completed payer debit including fees, or completed provider
  receipts excluding payer fees. Roles must not be summed together. Labelled
  `wallet_reported`; absence is not proof of nonpayment.

The allowlisted event includes no recovery ID, raw BOLT11, preimage, wallet
credentials, prompt, or response. Invoice refs are payment hashes and are
linkable financial metadata, not anonymous identifiers. This is local plugin
IPC, not public gossip or OTLP telemetry. Only trusted local plugins should
subscribe. Consumers must not promote peer-asserted correlation, provider claims,
or wallet observations into proof of correct inference.

`terms_digest` is lowercase SHA-256 of the UTF-8 JSON tuple:
`["mesh.payment.terms.v1", exchange_id, payee, model, input_msat_per_million,
output_msat_per_million, minimum_invoice_msat, input_tokens, max_output_tokens,
max_total_msat, expires_at_ms]`. Payer/provider-local peer fields and recovery
capabilities are excluded. `event_id` hashes the JSON tuple
`["mesh.payment.event.v1", exchange_id, role, phase, terms_digest, segment,
invoice_ref, amount_msat]`, allowing repeated observations to be deduplicated.

Delivery is bounded best-effort (256-event broadcast buffer and a one-second
plugin publication deadline), not an outbox or exactly-once feed. Durable state
is committed before observation; plugin failure cannot undo or block settlement.
Snapshots can repeat already-known phases. Recovery uses persisted correlation
when it observes new financial state, but missed events are not automatically
replayed to plugins after restart. Do not build a complete audit ledger by
assuming the plugin stream is lossless.

Compatibility: correlation travels inside the existing framed HTTP payload as
`x-mesh-payment-exchange-id`, not as a new field on the strict payment-v1 Request
frame. Optional `RequestTerms.exchange_id` is accepted by older terms readers;
old senders/records omit it. The local ledger upgrades to schema v2 to retain
provider terms; older binaries reject that newer schema rather than dropping it.
