...
Status: **LMCache-compatible CPU port implemented; acceptance rerun pending**.
Native passthrough remains the promoted path. Owner: jian yang.
...
|---|---|---|---|
| CPU (reference) | scalar Rust | **LMCache-compatible port implemented behind the gate** — per-token max scaling, layer-specific bins, per-channel CDFs, and arithmetic coding | Python/Rust fixtures pin LMCache revision `b5d109e` |
...
This is still an opt-in correctness path. It is not selected by storage or the
request path until a matched 19K run passes the existing size, TTFT, token
agreement, p99, and working-memory gates.

## Sequencing after this slice

1. Rerun the CacheGen quality/performance gate with the LMCache-compatible port
   versus native on the ~19K acceptance workload, using matched release builds.
