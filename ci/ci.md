...
| `agentic-replay-nightly.yml` | daily schedule, main + branch dispatch | Coding-agent replay benchmark on the persistent macOS `micstudio` runner. The fixed `[self-hosted, X64, macOS, family-certify, agentic-replay]` selector is backed by a fail-closed `RUNNER_NAME=micstudio` check; scheduled and main dispatches check out trusted `main`, while a branch dispatch checks out that branch — so only main runs publish to the HF dataset, and the write-capable `HF_TOKEN` is confined to the main-only upload step (the public dataset makes anonymous history download sufficient for gating; a `repo_info` probe distinguishes a genuinely missing dataset from auth/network failure so the gate fails closed). It resolves exact model and trajectory revisions from the pre-warmed Hugging Face cache, verifies SHA-256 digests, cross-checks the trajectory pin against `evals/skippy-competitive-benchmark.json` before download, derives the replay shape from `ci/agentic-replay-nightly/matrix.json`, uploads immutable evidence, gates every run against cohort-matched history, and may open a repair PR without executing pull-request content on the persistent runner. Branch dispatches use per-ref concurrency groups so they cannot queue-block the nightly. |
