...
The canary repair wrapper owns the target-pin transition: it writes
`third_party/llama.cpp/upstream.txt` and the `LLAMA_CPP_SHA` compatibility
mirror together, prepares through the checked-in `pinned` selector, and
verifies the prepared-upstream stamp before publishing or certifying a repair
branch. The hosted changed-pin job uses the same dual-pin updater.
