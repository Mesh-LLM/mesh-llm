...
- `ci-{linux,macos,windows}-product-smoke-slice.yml` — platform-local callers of the typed
  ...
  are ready for a sequenced catalog-only activation; the existing CUDA
  inference signal remains required until its typed product row passes live
  qualification. CUDA and Metal request
