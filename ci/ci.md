...
| `llama-upstream-canary.yml` | schedule, main-path push, dispatch | Trusted default-branch llama.cpp bump certification on the self-hosted `family-certify` runner; the workflow builds the certification binaries once, then the supported-families battery (single `ci/llama-canary/family-certified.tsv` manifest) gates the pin commit. The battery inspects GGUFs for native MTP heads, records model-size-aware startup deadlines and typed lane outcomes, and uploads all manifests/summaries/model scans/logs even on failure. Manual dispatch can force this certification when the upstream SHA is unchanged. Patch-apply failures trigger the opencode repair agent + repair PR. Runner reads its pre-warmed HF cache over NFS (`HF_CACHE` + `HF_HUB_OFFLINE=1` in the runner `.env`; no `flock` on NFS, so the runner never downloads) |
