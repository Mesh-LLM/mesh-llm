...
| `nightly-competitive-benchmark.yml` | daily schedule | Opt-in trusted-main CUDA scheduler benchmark on the fixed `gpu-nvidia` runner labels. Manual dispatch is intentionally disabled so a non-default ref cannot execute workflow content on the persistent runner. It builds the current `skippy-server`, consumes only pre-baked model/tokenizer/native/llama-benchy inputs named by `MESH_NIGHTLY_COMPETITIVE_*` repository variables, compares raw llama.cpp and fixed Mesh plus staged adaptive Mesh and capacity-matched, capability-detected vLLM/SGLang arms, uploads partial evidence for 30 days, and reports promotion candidates without mutating `main`. |
