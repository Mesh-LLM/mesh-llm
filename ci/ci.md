...
| `llama-upstream-canary.yml` | schedule, main-path push, dispatch | Trusted default-branch llama.cpp bump certification on the self-hosted `family-certify` runner; the workflow builds the certification binaries once, then the supported-families battery (single `ci/llama-canary/family-certified.tsv` manifest) gates the pin commit. Before any lane starts, the battery pins cached model paths to immutable snapshot SHAs, verifies all shard/tensor scans, disk headroom and certification ports, and runs a one-token MTP speculative-corpus smoke. Only GGUFs with MTP/NextN tensors run `llama-spec-bench`; every row retains core parity/dtype/state coverage. Model-size-aware startup deadlines, complete-certification wall-clock limits and typed lane outcomes are recorded, and immutable model manifests/preflight evidence/certification logs upload even on failure. Manual dispatch can force this certification when the upstream SHA is unchanged. Patch-apply failures trigger the opencode repair agent + repair PR. Runner reads its pre-warmed HF cache over NFS (`HF_CACHE` + `HF_HUB_OFFLINE=1` in the runner `.env`; no `flock` on NFS, so the runner never downloads) |
