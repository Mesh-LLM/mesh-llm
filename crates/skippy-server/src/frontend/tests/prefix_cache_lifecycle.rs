...
fn real_initialization_notifies_started_then_completed() {
    ...
    let kv = KvStageIntegration::from_config_with_observer(&config, Some(observer.clone()))
        .unwrap()
        .expect("resident prefix cache enabled");
    drop(kv);
...
fn disabled_kv_config_never_notifies_init_at_all() {
    ...
    let kv =
        KvStageIntegration::from_config_with_observer(&config, Some(observer.clone())).unwrap();
