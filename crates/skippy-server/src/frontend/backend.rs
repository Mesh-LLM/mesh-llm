    ...
    fn new(
        ...
        let entry = {
            ...
            let entry = locks
                ...
                .clone();
            // Keep acquisition of a registry reference atomic with lookup and
            // creation. Drop removes an entry while holding this same mutex;
            // incrementing after the lock is released could resurrect a lease
            // for an entry that has already been replaced.
            entry.users.fetch_add(1, Ordering::AcqRel);
            entry
        };
        Ok(Self {
...
fn trusted_generation_session_id(ids: &OpenAiGenerationIds) -> Option<&str> {
    ids.agent_session_id
        .as_deref()
        .filter(|_| ids.agent_session_trusted)
}
