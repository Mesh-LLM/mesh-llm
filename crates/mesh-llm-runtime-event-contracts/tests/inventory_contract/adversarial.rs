...
use crate::support::{
    Inventory, SpecManifest, inventory_event_ids, load_inventory, load_manifest, repo_root,
};
...
fn validate_acceptance(
    ...
    let package_names = fs::read_dir(root.join("crates"))
        ...
        .filter_map(|value| value["package"]["name"].as_str().map(str::to_owned))
        .collect::<BTreeSet<_>>();
    if inventory
        .families
        .iter()
    ...
    if inventory.spec_sha256 != format!("{:x}", Sha256::digest(spec)) {
