...
/// digest.
pub struct SidecarDigestCache {
    dir: PathBuf,
}

impl SidecarDigestCache {
    /// Open a cache rooted at an explicit caller-selected directory.
...
pub fn file_mtime_nanos(metadata: &fs::Metadata) -> Option<u128> {
...
/// alone.
#[cfg(unix)]
pub fn file_ctime_nanos(metadata: &fs::Metadata) -> Option<i128> {
...
#[cfg(not(unix))]
pub fn file_ctime_nanos(_metadata: &fs::Metadata) -> Option<i128> {
    None
}
...
mod tests {
    ...
    #[test]
    fn existing_v2_record_is_reused_without_rewriting_it() {
        let dir = tempfile::tempdir().unwrap();
        let entry = dir
            .path()
            .join("da10020746e023e6c101d6437a10e0f212b21f51d9ef1587b4d35c7a3115b493.json");
        let bytes = br#"{"version":2,"algo":"sha256","path":"/models/model.gguf","size":42,"mtime_nanos":1,"ctime_nanos":5,"digest":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}"#;
        fs::write(&entry, bytes).unwrap();
        let modified = entry.metadata().unwrap().modified().unwrap();
        let cache = SidecarDigestCache::open_in(dir.path().to_path_buf());
        assert_eq!(
            cache.lookup(Path::new("/models/model.gguf"), 42, 1, Some(5)),
            Some("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa".into())
        );
        assert_eq!(fs::read(&entry).unwrap(), bytes);
        assert_eq!(entry.metadata().unwrap().modified().unwrap(), modified);
    }
