...
#[test]
fn admitted_resident_names_bind_to_the_same_local_manifest() {
    ...
    let mut admission = skippy_protocol::StageAdmissionDescriptor::from(&planned);
    assert_eq!(
        crate::stage_load::admitted_resident_tensor_names(&admission, &manifest).unwrap(),
        vec!["native.layer.0", "shared"]
    );
    admission.package_id = format!("sha256:{}", "ff".repeat(32));
    assert!(
        crate::stage_load::admitted_resident_tensor_names(&admission, &manifest)
            .unwrap_err()
            .to_string()
            .contains("package identity differs")
    );
}

#[test]
fn admitted_resident_names_reject_unknown_tensor_ids() {
    let manifest = manifest();
    let planned = planned(&manifest, &["missing.tensor"], &["decode"]);
    let admission = skippy_protocol::StageAdmissionDescriptor::from(&planned);
    assert!(crate::stage_load::admitted_resident_tensor_names(&admission, &manifest).is_err());
}

#[test]
fn all_execution_profiles_must_agree_on_the_activation_frontier() {
    let manifest = manifest();
    let planned = planned(&manifest, &["package.tensor.a"], &["decode", "prefill"]);
    let mut admission = skippy_protocol::StageAdmissionDescriptor::from(&planned);
    assert!(crate::stage_load::admitted_activation_frontier(&admission).is_ok());
    admission.profiles[1]
        .activation_export_bindings
        .push("different-binding".into());
    assert!(
        crate::stage_load::admitted_activation_frontier(&admission)
            .unwrap_err()
            .to_string()
            .contains("disagree on activation frontier")
    );
    admission.profiles.clear();
    assert!(
        crate::stage_load::admitted_activation_frontier(&admission)
            .unwrap_err()
            .to_string()
            .contains("no execution profiles")
    );
}
