...
`source::planning` constructs the source-complete graph-planning manifest from verified GGUF identity. Mesh retains its profile-scoped source policy, remote package acquisition and serving hooks. Model acquisition and lifecycle orchestration remain outstanding.
