...
use skippy_cache::{
    HandoffManifest, HandoffSegmentRef, HandoffSegmentStore, KvFetchClient, SegmentCodecIdentity,
    ...
    for (index, chunk) in prefix.chunks(chunk_tokens).enumerate() {
        ...
        if let Some(store) = &store {
            ...
            segment_refs.push(HandoffSegmentRef {
                ...
                digest,
                codec_identity: Some(SegmentCodecIdentity::native_kv_page(
    ...
    if !recurrent.is_empty() {
        ...
        if let Some(store) = &store {
            ...
            segment_refs.push(HandoffSegmentRef {
                ...
                digest,
                codec_identity: Some(SegmentCodecIdentity::raw(recurrent.len() as u64)),
                meta_json: Some(serde_json::to_string(&PageSegmentMeta {
            ...
            frame_kind::RECURRENT => {
                ...
                if let Some(store) = store {
                    ...
                    segment_refs.push(HandoffSegmentRef {
                        ...
                        bytes: body.len() as u64,
                        digest,
                        codec_identity: Some(SegmentCodecIdentity::raw(body.len() as u64)),
                        meta_json: Some(serde_json::to_string(&PageSegmentMeta {
                            kind: "recurrent".to_string(),
                            kv_desc: None,
                            token_start: 0,
