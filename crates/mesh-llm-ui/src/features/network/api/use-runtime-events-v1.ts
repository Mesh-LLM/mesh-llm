...
type Attempt = 'progress' | 'transient' | 'fatal'
  ...
  useEffect(() => {
    ...
    const readStream = async (body: ReadableStream<Uint8Array>): Promise<Attempt> => {
      ...
      try {
        for (;;) {
          ...
          if (done || disposed) return accepted ? 'progress' : 'transient'
        ...
        return accepted ? 'progress' : 'transient'
      } finally {
    ...
    const run = async () => {
      ...
      while (!disposed) {
        ...
        failures = outcome === 'progress' ? 0 : failures + 1
