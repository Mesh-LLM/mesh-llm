...
/**
 * Decode one SSE block into its `id`/`event`/`data` fields. Comment blocks
 * (the frozen keepalive) and blocks without both an `event` and a `data`
 * line return `undefined`, so the caller skips them without treating the
...
export function parseSseBlock(block: string): SseChunk | undefined {
  ...
  if (event === undefined || lastEventId === undefined || data.length === 0) return undefined
