pub fn binary_repl(args: BinaryReplArgs) -> Result<()> {
    ...
    let tokenizer_plan = if tokenizer_materialized {
        None
    } else {
        Some(plan_gguf_stage_runtime_plan_for_range(
            tokenizer_path,
    ...
    let mut tokenizer_config = RuntimeConfig {
        stage_index: 0,
    ...
    if let Some(plan) = tokenizer_plan {
        plan.apply_to(&mut tokenizer_config);
    }
    let tokenizer = StageModel::open(tokenizer_path, &tokenizer_config)
    ...
    {
        let chat_template_plan =
            plan_gguf_stage_runtime_plan_for_range(&args.model_path, (0, 1), args.ctx_size, 1)?;
        let mut chat_template_config = RuntimeConfig {
            stage_index: 0,
            layer_start: 0,
            layer_end: 1,
            ctx_size: args.ctx_size,
        ...
        chat_template_plan.apply_to(&mut chat_template_config);
        let model = StageModel::open(&args.model_path, &chat_template_config)
