...
mod tests {
    ...
    fn write_hf_cache_gguf(revision: &str, file: &str) -> (CacheFixtureGuard, String, PathBuf) {
        ...
        let repo_root = huggingface_hub_cache_dir().join(huggingface_repo_folder_name(
            &repo_id,
    ...
    fn gpu_tune_local_resolver_handles_paths_cache_refs_configured_misses_and_duplicates() {
        ...
        let explicit = tune_resolver::resolve_explicit_tune_targets(
            &config,
            &[
        ...
        let configured = tune_resolver::resolve_configured_tune_targets(&config);
        ...
        assert_eq!(
            ...
            tune_resolver::TuneTargetSelection::Explicit { configured: false }
        );
        ...
        let cache_target = explicit
            ...
            .find(|target| {
                matches!(
                    target.local_source,
                    tune_resolver::LocalTargetSource::HuggingFaceCache { .. }
                )
            })
            .unwrap();
        ...
        match &cache_target.local_source {
            tune_resolver::LocalTargetSource::HuggingFaceCache { canonical_ref } => {
                assert_eq!(canonical_ref, &expected_identity.canonical_ref);
            }
            tune_resolver::LocalTargetSource::FilesystemPath { .. } => {
                panic!("expected cache target")
            }
        }
    }
    ...
    fn gpu_tune_rejects_remote_only_refs_without_download() {
        let resolution = tune_resolver::resolve_explicit_tune_targets_with_probe_for_tests(
            &MeshConfig::default(),
            &[
        ...
        assert_eq!(
            ...
            tune_resolver::TuneTargetResolveReason::RemoteRefRequiresDownload
        );
        assert_eq!(
            resolution.errors[1].reason,
            tune_resolver::TuneTargetResolveReason::NotFoundLocally
        );
