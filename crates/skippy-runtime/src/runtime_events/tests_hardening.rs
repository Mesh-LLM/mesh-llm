...
fn trampoline_catches_panicking_ingress_without_unwinding_across_ffi() {
    let registration =
        ModelOpenEventReporterRegistration::new(OperationId(1), |_event: RuntimeEvent| {
    ...
    // No panic should escape this call; catch_unwind inside the trampoline
    // absorbs it, matching the FFI boundary contract.
