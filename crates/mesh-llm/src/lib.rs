...
async fn run_cli_entrypoint() -> anyhow::Result<()> {
    ...
    usage_reporting::init_for_cli(&cli);
    ...
    if cli.command.as_ref().is_some_and(|c| {
        ...
        usage_reporting::record_cli_command(
            family,
        ...
        usage_reporting::shutdown().await;
        if command_dispatch? {
            return Ok(());
        }
    }

    let session = usage_reporting::ServeSession::start(&cli);
...
async fn emit_early_cli_process_event(
    ...
    usage_reporting::init_for_unparsed(config_path.as_deref());
    let logging_initialized =
    ...
    usage_reporting::record_cli_command(family, outcome);
    if logging_initialized {
        let _ = mesh_llm_host_runtime::shutdown_logging_for_one_shot_cli().await;
    }
    mesh_llm_commands::operational_logging::clear_cli_operational_audit_bridge();
