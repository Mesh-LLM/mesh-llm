...
pub(crate) async fn dispatch_doctor_command(
    ...
    match command {
        ...
        Some(DoctorCommand::Network { port, json }) => run_network_doctor(*port, *json).await,
        None => {
...
async fn run_network_doctor(port: u16, json_output: bool) -> Result<()> {
...
mod tests {
    use super::{
        SKIPPY_DIAGNOSTIC_ENDPOINTS, capture_skippy_native_log, network_report_lines,
