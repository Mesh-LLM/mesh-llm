...
/// from the config. An absent or unreadable config leaves the default, off.
fn apply_config_host_ram_offload(config_path: Option<&std::path::Path>) {
    let enabled = mesh_llm_host_runtime::command_support::plugin::load_config(config_path)
        .ok()
        .and_then(|config| config.gpu.host_ram_offload)
        .unwrap_or(false);
    mesh_llm_system::capacity::set_process_host_ram_offload(enabled);
}
