...
pub(crate) fn assert_expired_bootstrap_token_requires_matching_adopted_membership() {
    ...
    runtime.block_on(async {
        ...
        assert_direct_join_restores_policy(&policy, &expired, &adopted).await;
...
async fn assert_direct_join_restores_policy(
    ...
    for retry in [false, true] {
        ...
        crate::mesh::node_requirements::persist_adopted_mesh_membership(adopted, vec![])
...
fn accepted_peer_is_persisted_before_promotion() {
    tokio::runtime::Runtime::new().unwrap().block_on(async {
        ...
        let host = make_test_node(super::super::super::NodeRole::Worker)
            .await
            .unwrap();
        let joiner = make_test_node(super::super::super::NodeRole::Worker)
            .await
            .unwrap();
        ...
        let bytes = std::fs::read(
            crate::mesh::identity_persistence::adopted_mesh_membership_path().unwrap(),
        )
        .unwrap();
