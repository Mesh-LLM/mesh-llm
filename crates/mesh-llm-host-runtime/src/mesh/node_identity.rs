...
///
/// An [`PublicAddrSource::Observed`] address carries the NAT-mapped port
...
/// `BTreeSet`. A [`PublicAddrSource::LocallyEnumerated`] address only fills a
    ...
    match pub_addr.source {
        stun::PublicAddrSource::Observed => {
            addr.addrs.retain(|candidate| match candidate {
                TransportAddr::Ip(socket) => socket.ip() != pub_addr.addr.ip(),
                _ => true,
            });
