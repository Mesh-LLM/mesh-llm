...
/// Environment variables a downloaded provider executable is allowed to inherit.
///
/// The provider process is an untrusted, downloaded binary, so it must not see
/// the host environment. A denylist can only ever chase known secret names and
/// still leaks everything it has not heard of — `AWS_ACCESS_KEY_ID`,
/// `AWS_SESSION_TOKEN`, GCP/Azure keys, and organization-specific secrets. We
/// instead clear the environment and re-add only this small set of non-secret
/// variables the process genuinely needs to run and resolve resources.
...
/// Clear the child environment and re-add only the non-secret allowlist, so no
/// host credential is ever visible to the downloaded provider executable.
fn scrub_provider_environment(command: &mut Command) {
    command.env_clear();
    for (name, value) in allowlisted_provider_environment(std::env::vars_os()) {
        command.env(name, value);
    }
}

/// Filter a host environment down to the provider allowlist. Pure so credential
/// scrubbing can be tested without spawning a process or mutating global env.
fn allowlisted_provider_environment(
    env: impl IntoIterator<Item = (OsString, OsString)>,
) -> Vec<(OsString, OsString)> {
    env.into_iter()
        .filter(|(name, _)| {
            name.to_str()
                .is_some_and(|name| PROVIDER_ENV_ALLOWLIST.contains(&name))
        })
        .collect()
}
...
async fn observe_provider_health(
    ...
    match probe_provider(client, port, &runtime.model_ids).await {
        Ok(availabilities) => {
            state.failures = 0;
            ...
            if was_unrouted && !state.routed_model_ids.is_empty() {
                emit_provider_ready(runtime, &primary.1, port, pid);
            }
            None
        }
        Err(error) => {
...
async fn probe_provider(
    ...
    port: u16,
    model_ids: &[String],
) -> Result<Vec<(String, ProviderAvailability)>> {
...
mod tests {
    ...
    fn available_coreai_provider() -> ProviderAvailability {
        ProviderAvailability {
            ...
            versioned_model_id:
                "meshllm/qwen3-0.6b-4bit-aimodel@88064c009da71a9e5488c5db044fbfcf07703e42"
                    .to_string(),
            capabilities: vec!["reasoning".to_string(), "tool_calling".to_string()],
            ..available_provider()
        }
    }
    ...
    #[tokio::test]
    async fn one_provider_advertises_system_and_coreai_models() {
        let node = mesh::Node::new_for_tests(mesh::NodeRole::Host { http_port: 9_337 })
            .await
            .unwrap();
        let (target_tx, _target_rx) = watch::channel(election::ModelTargets::default());
        ...
        let availabilities = vec![
            ...
            (
                "meshllm/qwen3-0.6b-4bit-aimodel".to_string(),
                available_coreai_provider(),
            ),
        ];

        reconcile_provider_advertisements(&availabilities, &mut advertised, &context).await;
    ...
    #[test]
    fn provider_environment_allowlist_drops_host_credentials() {
        let host_env = vec![
            (os("PATH"), os("/usr/bin:/bin")),
            (os("HOME"), os("/home/mesh")),
            (os("AWS_ACCESS_KEY_ID"), os("AKIA...")),
            (os("AWS_SESSION_TOKEN"), os("secret-session")),
            (os("GOOGLE_APPLICATION_CREDENTIALS"), os("/creds.json")),
            (os("AZURE_CLIENT_SECRET"), os("azure-secret")),
            (os("ACME_ORG_SECRET"), os("nope")),
            (os("OPENAI_API_KEY"), os("sk-...")),
        ];

        let scrubbed = allowlisted_provider_environment(host_env);
        let names: Vec<&str> = scrubbed
            .iter()
            .map(|(name, _)| name.to_str().unwrap())
            .collect();

        // Only the non-secret allowlist survives.
        assert_eq!(names, vec!["PATH", "HOME"]);

        // Nothing outside the allowlist — especially credentials — is forwarded,
        // including names the old denylist never enumerated.
        for name in [
            "AWS_ACCESS_KEY_ID",
            "AWS_SESSION_TOKEN",
            ...
            assert!(
                !names.contains(&name),
                "credential {name} must not reach the provider process"
            );
        }
    }

    #[test]
    fn provider_environment_allowlist_only_admits_known_names() {
        let scrubbed = allowlisted_provider_environment(
