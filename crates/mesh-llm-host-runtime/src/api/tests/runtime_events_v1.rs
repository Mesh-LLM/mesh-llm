...
mod runtime_event_api {
    ...
    async fn header_cursor_wins_over_query_cursor_and_replays_only_the_in_window_tail() {
        ...
        // Submit two terminal facts (sequences 0 and 1).
        for _ in 0..2 {
        ...
        // Header says "I've seen sequence 0"; the stale query cursor of "-1
        // equivalent" (sequence 0 too, doesn't matter — header must win).
        let header_cursor = format!("rt1:{}:0", engine.process_instance().as_uuid());
        ...
        // Only sequence 1's frame should replay (sequence 0 was already seen).
        assert_eq!(text.matches("event: runtime_event").count(), 1, "{text}");
