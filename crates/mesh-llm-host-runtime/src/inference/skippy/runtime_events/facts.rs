...
/// Maps successful/cancelled termination onto the family's terminal kinds.
/// `GenerationReceiptSink::record` is only reachable on a successful local
/// generation (`GenerationReceipt` is target-authoritative success
/// evidence); `Aborted` covers the failure/cancellation path separately, so
/// every `GenerationTermination` variant here maps to a completed outcome.
/// `outcome` reflects the ACTUAL termination, not a blanket `Success`:
...
pub(super) fn receipt_terminal_fact(receipt: &GenerationReceipt) -> RuntimeFact {
    ...
    let (kind, outcome) = match receipt.termination {
    ...
    let data = FactData {
        ...
        numeric_summaries: token_count_summaries(&[
            ("generated_token_count", redacted.generated_token_count),
            ("prompt_token_count", redacted.prompt_token_count),
        ]),
        ..empty_data()
    };
...
/// `GenerationReceipt::request_to_first_token_us` field: the target-
/// authoritative backend-request-start-to-first-token latency IS the
/// existing signal for "prefill completed" (decode begins at first token).
/// A generation that never emitted a token (`None`) reports the same
/// terminal without the timing summary rather than inventing a failure this
/// layer cannot actually distinguish from "zero tokens requested".
pub(super) fn prefill_terminal_fact(receipt: &GenerationReceipt) -> RuntimeFact {
    let data = match receipt.request_to_first_token_us {
