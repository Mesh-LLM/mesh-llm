...
fn emit_stage_set_and_capacity_changed(topology_id: &str, stage_id: &str, lane_count: u32) {
    let Some(engine) = runtime_event_engine() else {
        return;
    };
    let scope = stage_scope(topology_id, stage_id, 0);
...
impl StageLoadOperation {
    ...
    pub(crate) fn ready(mut self, lane_count: u32) {
        ...
        emit_stage_set_and_capacity_changed(&self.topology_id, &self.stage_id, lane_count);
    }
...
impl StageStopOperation {
    ...
    pub(crate) fn stopped(mut self) {
        ...
        emit_stage_set_and_capacity_changed(&self.topology_id, &self.stage_id, 0);
    }
}

#[cfg(test)]
mod tests {
    use super::{StageLoadOperation, StageStopOperation};
    use crate::runtime_events::engine::RuntimeEventEngine;
    use crate::runtime_events::{clear_runtime_event_engine, install_runtime_event_engine};
    use mesh_llm_runtime_event_contracts::{
        ReasonCode, RuntimeEventIngress, RuntimeFact, StageTopologyEventKind,
    };
