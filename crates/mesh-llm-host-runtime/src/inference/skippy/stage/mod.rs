...
use status::{
    failed_status_from_load, preparation_status_from_cancel, preparation_status_from_load,
    status_from_running, stopped_status,
};
pub(crate) use types::*;
...
impl StageControlState {
    ...
    async fn load(&mut self, load: StageLoadRequest) -> Result<StageReadyResponse> {
        match load.backend.as_str() {
            "skippy" => {}
            #[cfg(all(feature = "mlx", target_os = "macos"))]
            "mlx" => {}
            backend => anyhow::bail!("unsupported stage backend '{backend}'"),
        }
