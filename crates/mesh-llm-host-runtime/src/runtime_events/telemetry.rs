...
//!
//! Every push here is non-blocking by construction: [`RuntimeEventTelemetryQueue::push`]
//! only ever takes a short `Mutex` critical section over a bounded
//! `VecDeque` (drop-oldest on overflow, counted so the OTLP consumer can
//! export it as its own "telemetry health" signal) and never calls back
...
impl RuntimeEventTelemetryQueue {
    ...
    /// Push `sample`, dropping the oldest queued sample first if the queue
    /// is already at capacity. Never blocks beyond a short lock: no I/O, no
    /// call into any `RuntimeEventIngress`.
