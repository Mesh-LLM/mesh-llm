...
| `skippy.activation_wire_dtype` | enum | `auto` (default), `f16`, `f32`, `q8` | both | model reload | wired (staged mode only) | none |
...
Staged-only controls stay staged-only: activation wire dtype, lifecycle
intervals, prefill controls, and manual stage layer ranges only take effect
...
its eventual typed per-model TOML replacement.
