...
if [ -n "$PROMPT_SUITE_JSON" ]; then
    ...
    python3 - "$PROMPT_SUITE_JSON" <<'PY' | while IFS=$'\t' read -r label prompt expect_contains expect_contains_ci expect_contains_all_ci_json expect_any_ci_json forbid_contains expect_exact max_tokens; do
import json, sys
...
PY
        ...
        run_chat_case "$prompt" "$expect_contains" "$expect_contains_ci" "$expect_contains_all_ci_json" "$expect_any_ci_json" "$forbid_contains" "$expect_exact" "$label" "$max_tokens"
    done
fi
