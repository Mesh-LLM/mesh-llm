...
def pair_metric_samples(
    ...
    from this metric's pairing -- pairwise only, never resampled, never
...
def relative_degradation(baseline_value: float, candidate_value: float, direction: str) -> float:
    ...
    zero is treated as an unusable pair by the caller before reaching here
    (comparator excludes it during pairing, not here)."""
...
def screen_metric(
    ...
    *,
    ...
    metric_seed = (seed ^ hash((group, metric))) & ((1 << 64) - 1)
    mean, ci_low, ci_high, resample_means = bootstrap_ci(deltas, bootstrap_samples, metric_seed)
