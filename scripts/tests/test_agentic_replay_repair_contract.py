...
''')
    ...
    def test_persistent_repair_only_emits_publication_data(self) -> None:
        ...
        self.assertIn(
            "run_untrusted python3 scripts/agentic-replay-params.py", self.repair
        )
        self.assertIn(
            "run_untrusted python3 scripts/agentic-replay-history.py", self.repair
        )
        ...
        self.assertIn(
            'git diff --cached --quiet "$BASE_SHA" -- .github .agents scripts evals ci',
            self.repair,
        )
        ...
        self.assertIn(
            "git -c core.hooksPath=/dev/null commit --no-gpg-sign", self.repair
        )
        ...
        self.assertIn(
            'if [[ ! -d "$OUTPUT_DIR" || -L "$OUTPUT_DIR" ]]; then', self.repair
        )
    ...
    def test_repair_rerun_uses_shared_matrix_command(self) -> None:
        self.assertIn("python3 scripts/agentic-replay-params.py", self.repair)
        self.assertIn('--matrix "$MATRIX_FILE" --run-family "$family"', self.repair)
        self.assertNotIn("--trajectories-per-framework", self.repair)
    ...
    def test_workflow_uses_full_session_matrix_contract(self) -> None:
        self.assertIn('MODE_MAP = {"all": "all"}', self.params)
        self.assertIn('--github-env "$GITHUB_ENV"', self.workflow)
        self.assertEqual(self.matrix["replay"]["mode"], "all")
        self.assertIn(
            '--replay "$RUNNER_TEMP/agentic-replay-params.json"', self.workflow
        )
        for model in self.matrix["models"]:
            self.assertIn("--run-family " + model["family"], self.workflow)
        self.assertIn('--run-family "$family"', self.repair)
