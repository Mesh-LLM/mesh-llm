...
// Re-export public types and functions so the rest of the crate can use
// `mesh::NodeRole`, `mesh::PeerInfo`, etc. without knowing the submodule.
pub use self::identity::{
    clear_public_identity, generate_mesh_id, load_last_mesh_id, mark_was_public, save_last_mesh_id,
    was_previously_public,
};
pub use self::inventory::{
    detect_vram_bytes_capped, scan_all_model_metadata, scan_local_model_sizes,
};
pub use self::peer::{
    merge_demand, ModelDemand, NodeRole, PeerInfo, DEMAND_TTL_SECS, MAX_SPLIT_RTT_MS,
};
pub(crate) use self::peer::{PeerAnnouncement, PeerAnnouncementV0};

// Internal helpers from peer submodule used by the Node implementation.
...
use self::peer::{
    ...
    peer_meaningfully_changed, PEER_STALE_SECS,
};
