...
pub(crate) struct Cli {
    ...
    /// MoE ranking strategy used when split mode is active.
    #[arg(long, value_enum, default_value = "auto", hide = true)]
    pub(crate) moe_ranking: crate::inference::moe::MoeRankingStrategy,

    /// Prompt count for micro-analyze ranking.
    #[arg(long, default_value = "1", hide = true)]
    pub(crate) moe_micro_prompts: usize,

    /// Token count per prompt for micro-analyze ranking.
    #[arg(long, default_value = "8", hide = true)]
    pub(crate) moe_micro_tokens: u32,

    /// Layer scope for micro-analyze ranking.
