//! `ExternalPlugin` and `PluginRuntime` — per-plugin process management and I/O loop.
//!
...
impl ExternalPlugin {
    ...
    pub(super) async fn supervise(&self) -> Result<()> {
    ...
    pub(super) async fn server_info(&self) -> Result<ServerInfo> {
        self.ensure_running().await?;
        self.server_info
            .lock()
            .await
    ...
    pub(super) async fn list_tools(&self) -> Result<Vec<ToolSummary>> {
    ...
    pub(super) async fn mcp_request<T, P>(&self, method: &str, params: P) -> Result<T>
    where
    ...
    pub(super) async fn mcp_notify<P>(&self, method: &str, params: P) -> Result<()>
    where
        P: Serialize,
    {
    ...
    pub(super) async fn send_channel_message(&self, message: proto::ChannelMessage) -> Result<()> {
        self.send_unsolicited(
    ...
    pub(super) async fn send_bulk_transfer_message(
        &self,
    ...
    pub(super) async fn send_mesh_event(&self, event: proto::MeshEvent) -> Result<()> {
