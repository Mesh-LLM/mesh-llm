...
Roles are exchanged via gossip over protobuf on QUIC ALPN `mesh-llm/1`, with `meshllm.node.v1` for mesh state and `meshllm.config.v1` for owner-gated config sync. A node transitions Worker → Host when elected.
...
The control plane uses QUIC ALPN `mesh-llm/1` with split protobuf schemas: `meshllm.node.v1` for mesh state and `meshllm.config.v1` for config sync. Scoped control-plane streams use 4-byte LE framing followed by protobuf bytes.
