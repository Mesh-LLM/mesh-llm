...
This document describes the wire protocol for control-plane communication between mesh-llm nodes over QUIC ALPN `mesh-llm/1`, using split protobuf schemas: `meshllm.node.v1` for mesh state and `meshllm.config.v1` for owner-gated config sync.
